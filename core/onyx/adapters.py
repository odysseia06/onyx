...
class ResolvedAgent:
    ...
    def __init__(self, agent: AgentDef, ctx: RenderContext, enabled_modules: set[str], *, origin: str) -> None:
        ...
        rt = lambda text: render_text(text, ctx, origin=origin)  # noqa: E731
...
def agents_md_intent(
    ...
    lines = [
        "# AGENTS.md — generated by Onyx",
        "",
        "Instructions for agent runtimes operating this Obsidian vault. Generated from the enabled module set; regenerated when it changes — if you edit this file it becomes yours and updates will land beside it as `AGENTS.md.new`.",
        "",
        "## Vault conventions (the short version)",
        "",
        "- Framework-created notes carry `type`, `created` (ISO date, set once), `status` (per-type lifecycle), and `tags` (user-owned). Preserve unknown frontmatter keys byte-for-byte; never validate or reformat the user's own notes.",
        "- Wikilinks for internal references; plain markdown links only for external URLs. Do not hard-wrap prose — one logical line per paragraph or bullet.",
        "- `{{...}}` placeholders were the engine's and are already resolved; `<% tp.* %>` belongs to Templater — leave it exactly as written, or substitute real values when instantiating a template by hand.",
        "- Files tracked in `.vault/lock.json` are framework-managed: editing one hands it to the user and future updates arrive as `*.new` siblings. Never move, rename, or delete files you did not create; never touch `.vault/`.",
        "- The full rules ship as skill packages under `.claude/skills/` (`vault-conventions` first); read them, they are plain markdown.",
        "",
        "## Enabled modules",
        "",
    ]
    ...
    if skill_ids:
        lines += [
            ...
            "## Skill packages",
            "",
            "Workflow instructions, one folder per skill under `.claude/skills/` (plain markdown, runtime-agnostic):",
            "",
        ]
...
def claude_orientation_intents(
    ...
    managed ``.claude/onyx.md`` digest it imports.

    ``CLAUDE.md`` is loaded automatically every session, so it is where an agent
    learns it is in an Onyx vault, how to drive it through the ``obsidian`` CLI,
