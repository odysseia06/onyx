"""`onyx project new` — instantiate a projects-software project from its template.
...
never recorded in lock.json — `onyx update`/`remove` must never reconcile it.
The subtree is rendered from the module ASSETS (raw `{{onyx.today}}`), so dates
...
from .errors import OnyxError, PathError
...
def scaffold_project(vault_root: Path, name: str, modules_root: Path, *, today: str) -> str:
    ...
    if _PROJECT_MODULE not in config.modules:
        raise OnyxError(
            ...
            f"enable it with `onyx add {_PROJECT_MODULE}` first"
        )
    try:
        segments = split_portable(name)
    except PathError as exc:
        raise OnyxError(f"{name!r} is not a valid project name: {exc}") from None
    if len(segments) != 1:
        raise OnyxError(f"{name!r} must be a single folder name, not a path")
    ...
    template_root = render_path(
        ...
        is_file=False, origin="onyx project new",
    )
