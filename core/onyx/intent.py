...
deterministic for a given (config, module library, ONYX_NOW). The planner
...
def resolve_today() -> str:
    """Today's ISO date, overridable via ONYX_NOW so tests and CI stay byte-exact."""
...
def build_desired_state(config: Config, manifests: list[Manifest]) -> DesiredState:
    ...
    from .adapters import agents_md_intent, claude_code_intents  # local import: adapters builds FileIntents from here
